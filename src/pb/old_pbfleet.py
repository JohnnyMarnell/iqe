...
PROVISION_STATE_FILE = Path("provisioned_devices.json")
...
class ProvisioningManager:
    ...
    def load_provisioned(self) -> set:
        ...
        if PROVISION_STATE_FILE.exists():
            try:
                with open(PROVISION_STATE_FILE, 'r') as f:
                    data = json.load(f)
    ...
    def save_provisioned(self):
        ...
        try:
            with open(PROVISION_STATE_FILE, 'w') as f:
                json.dump({
...
allowed_origins = os.environ.get('CORS_ORIGINS', 'http://localhost:*,http://127.0.0.1:*,http://192.168.*:*,http://10.*:*').split(',')
app.add_middleware(
    CORSMiddleware,
