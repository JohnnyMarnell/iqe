...
const osc = new OSC({
	plugin: new OSC.WebsocketClientPlugin({ port: 8080, host: "iqe.local" }),
});
...
function receiveOsc(msg) {
	...
	if (selections) {
		...
		selections
			.filter((el) => el.arg == msg.args[0])
			.forEach((el) => el.classList.add("selected"));
	}

	let first = msg.args[0];
	try {
		first = JSON.parse(msg.args[0]);
	} catch (e) {}
	handle(msg.address, first, msg.args);
}
...
"path arg select"
	.split(" ")
	.forEach((prop) => oscElements.forEach((el) => (el[prop] = attr(el, prop))));
oscElements
	.filter((el) => el.path)
	.forEach((el) => (pathToElement[el.path] = el));
...
all('[type="range"]').forEach((el) =>
	el.addEventListener("input", (_) => send(el.path, parseFloat(el.value))),
);

// Handle text / type box inputs
all('[type="text"]').forEach((el) =>
	el.addEventListener("change", (_) => send(el.path, parseFloat(el.value))),
);
...
all(".button").forEach((el) => {
	...
	if (on !== undefined && off !== undefined) {
		...
		el.addEventListener(
			"touchstart",
			(e) => send(el.path, on) || e.preventDefault(),
		);
		el.addEventListener(
			"touchend",
			(e) => send(el.path, off) || e.preventDefault(),
		);
	} else {
		...
		el.addEventListener(
			"touchstart",
			(e) => send(el.path, arg) || e.preventDefault(),
		);
	}
});
