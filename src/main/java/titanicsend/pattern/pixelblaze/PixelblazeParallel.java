...
public class PixelblazeParallel extends PortFacade_TEPerformancePattern {
  ...
  public PixelblazeParallel(LX lx) {
    ...
    try {
      ...
      int chunksize = (int) Math.ceil((float) this.model.points.length / N_THREADS);
      List<LXPoint> all = Arrays.asList(model.points);
      for (int i = 0; i < all.size(); i += chunksize) {
        List<LXPoint> chunk = all.subList(i, Math.min(i + chunksize, all.size() - 1));
        LXPoint[] chunkPoints = new LXPoint[chunk.size()];
        wrappers.add(Wrapper.fromResource("neon_ice", this, all.toArray(chunkPoints)));
      }
...
//        List<LXPoint> chunk = modelTE.edgePoints.subList(i, Math.min(i + chunksize, modelTE.edgePoints.size() - 1));
//        LXPoint[] chunkPoints = new LXPoint[chunk.size()];
...
//        List<LXPoint> chunk = modelTE.panelPoints.subList(i, Math.min(i + chunksize, modelTE.panelPoints.size() - 1));
//        LXPoint[] chunkPoints = new LXPoint[chunk.size()];
