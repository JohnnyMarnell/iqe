...
/***
 * Not sure how to properly do this, but started with decompiling packaged Chromatik app,
 * renamed to ChromatikIQE, changd pom to java 17, fixed throwable + final errors with lambda,
 * added initHook.
 */
public final class ChromatikIQE extends LXStudio {
    ...
    /** Added! */
    private void initHook(ChromatikIQE chromatik) {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (headless) {
                ...
                final File projFile = projectFile;
                lx.engine.addTask(() -> {
                    if (isSchedule) {
                        lx.preferences.schedulerEnabled.setValue(true);
                        LX.log("Opening schedule file: " + projFile);
                        lx.scheduler.openSchedule(projFile, true);
                    } else {
                        try {
                            lx.preferences.loadInitialProject(projFile);
